        ...
        if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    Eigen::VectorXd ptx = Eigen::Map<Eigen::VectorXd>(ref_track.x.data(),ref_track.x.size());
                    Eigen::VectorXd pty = Eigen::Map<Eigen::VectorXd>(ref_track.y.data(),ref_track.y.size());
                    auto coeffs = polyfit(ptx,pty,3) ;

                    // calculate the cross track error : at 0 because we are in veh coordinates
                    auto cte = polyeval(coeffs,0);
                    // calculate the orientation error
                    double epsi = - atan(coeffs[1]);

                    Eigen::VectorXd state(6);
                    //Pos at 0 because we are in veh coordinates
                    state << 0, 0, 0, v, cte, epsi;
                    auto vars = mpc.Solve(state,coeffs);
                    double steer_value(vars[12]);
                    double throttle_value(vars[13]);
                    ...
                    //Display the MPC predicted trajectory : already in veh coordinates
                    vector<double> mpc_x_vals{vars[0],vars[2],vars[4],vars[6],vars[8],vars[10]};
                    vector<double> mpc_y_vals{vars[1],vars[3],vars[5],vars[7],vars[9],vars[11]};
